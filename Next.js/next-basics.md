...
```ts
const country =
  request.geo?.country ?? process.env.NEXT_PUBLIC_GEO_FALLBACK ?? "US";
```
...
export async function generateMetadata({ params, searchParams }) {
  ...
  // Fetch product data
  const product = await fetch(`https://example.com{id}`).then((res) =>
    res.json(),
  );
...
export default function AdminPanelCard() {
  ...
  return (
    <div className="admin-card">
      ...
      <button onClick={() => alert("Flushing server logs...")}>
...
export function middleware(request: NextRequest) {
  const bucket =
    request.cookies.get("ab-checkout")?.value ??
    (Math.random() < 0.5 ? "a" : "b");
