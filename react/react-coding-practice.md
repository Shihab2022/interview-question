...
export async function submitContact(
  ...
  return {
    success: true,
    message: `Thank you, ${name}! We'll be in touch soon.`,
  };
}
```
...
function Solution() {
  ...
  const handleIncrementCorrect = () => {
    ...
    alert(
      `Correct approach - Counter value in alert: ${correctCounter} (old value)`,
    );
  };
...
function Solution() {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <MemoOnly onClick={handleClickWrong} />{" "}
      {/* still re-renders — function ref changes */}
      <OptimizedChild onClick={handleClickCorrect} />
